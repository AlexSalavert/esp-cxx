...
#ifdef CONFIG_EXAMPLE_WEATHER_MONITORING
...
#elif CONFIG_EXAMPLE_HIGH_PRECISION
    #define BMX280_DEFAULT_CONFIG esp_cxx::Bmx280::Config::highPrecision()
#endif

#define MEASURING_INTERVAL_MS (uint16_t)CONFIG_EXAMPLE_BMX280_MEASURING_INTERVAL_MS
...
extern "C" void app_main()
{
    ...
    if(sensor.set_config(BMX280_DEFAULT_CONFIG) == ESP_OK){
        ESP_LOGI(TAG, "set sensor configurate OK");
    }else{
        ESP_LOGE(TAG, "set sensor configurate ERROR");
    }

    if(sensor.get_config(config) == ESP_OK){
        ...
        ESP_LOGE(TAG, "get sensor configurate ERROR");    
    }
    ...
    while(true){
        ...
        vTaskDelay(pdMS_TO_TICKS(MEASURING_INTERVAL_MS));
        printf("\n");

        if(sensor.read_temperature(temp) == ESP_OK){
            ESP_LOGI(TAG, "Temperature = %.2f °C", temp);
        }else{
            ESP_LOGE(TAG, "read temperature ERROR"); 
        }
        
        if(sensor.read_pressure(press) == ESP_OK){
            ESP_LOGI(TAG, "Pressure    = %.2f hPA ", press / 100.0f);
        }else{
            ESP_LOGE(TAG, "read pressure ERROR"); 
        }
        
        #if(HUMIDITY_ENABLE)
            if(sensor.read_humidity(hum) == ESP_OK){
                ESP_LOGI(TAG, "Humidity    = %.2f %%", hum);
            }else{
                ESP_LOGE(TAG, "read humidity ERROR"); 
            }
            
        #endif

        if(sensor.read_altitude(alt) == ESP_OK){
            ESP_LOGI(TAG, "Altitude    = %.2f m", alt);
        }else{
            ESP_LOGE(TAG, "read altitude ERROR"); 
        }
        
        if(sensor.read(data) == ESP_OK){
            ESP_LOGI(TAG, "TEMP = %.2f °C | PRESS = %.2f hPA | HUM = %.2f %% | ALT = %.2f m", data.temp, data.press / 100.0f, data.hum, data.alt);
        }else{
            ESP_LOGE(TAG, "read data ERROR"); 
        }
    }
}
